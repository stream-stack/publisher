...
func TestXdsServer_StoreSetPush(t *testing.T) {
	...
	dial, err := grpc.Dial("127.0.0.1:8080", grpc.WithInsecure(), grpc.WithUnaryInterceptor(grpc_retry.UnaryClientInterceptor(retryOpts...)))
	if err != nil {
		panic(err)
	}
	...
	subscribes[0] = &proto.Subscribe{Name: "test1"}
